...
if [[ -f "$SELF_DIR/deploy/monitor/db-check.sh" ]]; then
  export BW_GITHUB_REPO="$REPO" BW_GITHUB_REF="$REF"
  exec bash "$SELF_DIR/deploy/monitor/db-check.sh"  "$@"
fi
...
[[ -f "$ROOT/deploy/monitor/db-check.sh" ]] || { echo 'Downloaded repository is incomplete.' >&2; exit 1; }
export BW_GITHUB_REPO="$REPO" BW_GITHUB_REF="$REF"
exec bash "$ROOT/deploy/monitor/db-check.sh"  "$@"
