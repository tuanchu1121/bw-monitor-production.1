...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bw-monitor-v48104-test-") as tmp:
        ...
        check(appmod.app.view_functions["vm_abuse_page"].__name__ in {"vm_abuse_page_v48103", "vm_abuse_page_v48126", "vm_abuse_page_v48128", "vm_abuse_page_v48129", "vm_abuse_page_v48131"}, "guest-aware abuse/intelligence view is not active")
        check(appmod.app.view_functions["vm_page"].__name__ in {"vm_page_v48103", "vm_page_v48131"}, "guest-aware VM detail view is not active")
        ...
        with appmod.app.test_request_context("/top?sort=ram&order=desc&limit=1000"):
            ...
            check(sum(top_html.count(f">RAM{suffix}</a>") for suffix in ("", " ↓", " ↑")) == 1, "Top VM should keep one RAM column")
