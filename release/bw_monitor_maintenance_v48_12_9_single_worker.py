#!/usr/bin/env python3
"""Out-of-process FIFO database maintenance runner for BW Monitor v48.12.9.
...
def job_conn(db_path: str) -> sqlite3.Connection:
...
def checkpoint_database(db_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"checkpoint": list(row) if row else []}
    finally:
        conn.close()
...
def vacuum_database(db_path: str) -> dict[str, Any]:
    ...
    parent = str(Path(db_path).resolve().parent)
    free_before = shutil.disk_usage(parent).free
    required_free = before + VACUUM_FREE_RESERVE
    if before > 0 and free_before < required_free:
        raise RuntimeError(
            f"Not enough free disk space for safe VACUUM: free={free_before} bytes, "
            f"required~={required_free} bytes (DB size plus reserve). Delete history "
            "without compacting or free disk space first."
        )
    checkpoint_database(db_path)
    conn = sqlite3.connect(db_path, timeout=3600, isolation_level=None)
    try:
...
def _transactional_purge(module, action: str, params: dict[str, Any]) -> dict[str, Any]:
    ...
    job may contain a larger selection, but each item gets its own short SQLite
...
def _start_service_reliably(service: str, attempts: int = 3) -> None:
    ...
    because the first systemd start raced with SQLite/WAL cleanup.
    """
        ...
        if job_id > 0:
            try:
                update_job(
                    ...
                    job_id,
                    "running",
                    "History deletion completed. Compacting SQLite; dashboard is temporarily offline.",
                )
            except BaseException:
                pass
        _offline_vacuum(db_path, app_service, result)
