#!/usr/bin/env python3
"""Regression coverage for v48.13.4 storage precision.
...
def main():
    ...
    check('V48134_VERSION = "48.13.4"' in source, "v48.13.4 marker is missing")
    ...
    check('Storage Node' in source and 'storage-disk-detail-table' in source and 'storage-one-disk' in source, "one-row-per-disk Storage I/O UI is missing")
    ...
    with tempfile.TemporaryDirectory(prefix="bw-storage-test-") as td:
        ...
        check(disk_html.count("storage-one-disk") >= 2 and "vda" in disk_html and "vdb" in disk_html, "VM disks are not rendered as separate rows")
    ...
    print("PASS: v48.13.4 exact UUID purge, abuse preservation, Top VM capacity, VM disk detail, per-disk Storage I/O, admin filters and separate /home mounts")
    return 0
