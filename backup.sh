...
if [[ -f "$SELF_DIR/deploy/monitor/backup.sh" ]]; then
  export BW_GITHUB_REPO="$REPO" BW_GITHUB_REF="$REF"
  exec bash "$SELF_DIR/deploy/monitor/backup.sh"  "$@"
fi
...
[[ -f "$ROOT/deploy/monitor/backup.sh" ]] || { echo 'Downloaded repository is incomplete.' >&2; exit 1; }
export BW_GITHUB_REPO="$REPO" BW_GITHUB_REF="$REF"
exec bash "$ROOT/deploy/monitor/backup.sh"  "$@"
