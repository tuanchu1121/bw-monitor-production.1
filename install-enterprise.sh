...
if [[ -x "$SELF_DIR/deploy/enterprise/install-enterprise.sh" ]]; then exec bash "$SELF_DIR/deploy/enterprise/install-enterprise.sh" "$@"; fi
...
[[ -x "$ROOT/deploy/enterprise/install-enterprise.sh" ]] || { echo 'Downloaded repository has no Enterprise installer.' >&2; exit 1; }
exec bash "$ROOT/deploy/enterprise/install-enterprise.sh" "$@"
