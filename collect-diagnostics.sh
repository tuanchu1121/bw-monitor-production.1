#!/usr/bin/env bash
set -Eeuo pipefail
REPO="${BW_GITHUB_REPO:-tuanchu1121/bw-monitor-production}"
REF="${BW_GITHUB_REF:-main}"
TOKEN="${GITHUB_TOKEN:-}"
TMP=""
trap '[[ -n "$TMP" ]] && rm -rf "$TMP"' EXIT
[[ "$(id -u)" == "0" ]] || { echo "Run as root or through sudo." >&2; exit 1; }
SELF_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]:-$PWD}")" 2>/dev/null && pwd || true)"
if [[ -f "$SELF_DIR/deploy/monitor/collect-diagnostics.sh" ]]; then
  export BW_GITHUB_REPO="$REPO" BW_GITHUB_REF="$REF"
  exec bash "$SELF_DIR/deploy/monitor/collect-diagnostics.sh"  "$@"
fi
...
[[ -f "$ROOT/deploy/monitor/collect-diagnostics.sh" ]] || { echo 'Downloaded repository is incomplete.' >&2; exit 1; }
export BW_GITHUB_REPO="$REPO" BW_GITHUB_REF="$REF"
exec bash "$ROOT/deploy/monitor/collect-diagnostics.sh"  "$@"
